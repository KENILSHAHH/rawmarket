...
import {
  ...
  ChevronDown, CircleAlert, Clock3, ExternalLink, Info, Landmark, Menu,
  RefreshCw, Search, Server, ShieldCheck, SlidersHorizontal, WalletCards, X,
} from 'lucide-react';
...
function CandleChart({market,data,timeframe,onTimeframe}:{market:Market;data:CandleResponse;timeframe:string;onTimeframe:(value:string)=>void}){
  ...
  return <section className="chart-card panel">
    <div className="panel-heading chart-heading"><div><h2>{market.symbol} / USD</h2><span>Reference index · candlestick</span></div><div className="chart-legend"><span className="status-dot"/> {data.source_status}</div></div>
...
function OrderBook({book,symbol,onPrice}:{book:Book|null;symbol:string;onPrice:(price:number)=>void}){
  ...
  return <section className="book-card panel"><div className="panel-heading"><div><h2>Order book</h2><span>Live AWS CLOB</span></div><span className="sequence">SEQ {book?.sequence||'—'}</span></div><div className="book-columns"><span>Price (USD)</span><span>Size ({symbol})</span><span>Total</span></div><div className="book-side asks">{[...asks].reverse().map(level=>row(level,'ask'))}</div><div className="spread-row"><div><strong>{book?.last.toFixed(2)||'—'}</strong><span>mid</span></div><div><strong>{spread?spread.toFixed(2):'—'}</strong><span>spread</span></div></div><div className="book-side">{bids.map(level=>row(level,'bid'))}</div></section>;
}
...
export default function Home(){
  ...
  const refreshAccount=useCallback(async()=>{
    ...
    }catch{/* Connectivity is represented by the engine badge. */}
  },[wallet]);
  ...
  const connect=async()=>{if(!wallet)return;setSubmitting(true);try{const next=await request<Account>('/api/fund',{method:'POST',body:JSON.stringify({wallet,amount:10000})});setAccount(next);setConnected(true);setToast({kind:'success',message:'Test wallet funded with $10,000 demo USD'})}catch(error){setToast({kind:'error',message:error instanceof Error?error.message:'Funding failed'})}finally{setSubmitting(false)}};
  const submit=async()=>{
    ...
    setSubmitting(true);setToast({kind:'info',message:'Submitting order to the AWS matching engine…'});
    try{
  ...
  return <main className="app-shell">
    <header className="topbar"><div className="brand"><div className="brand-glyph"><span/><span/><span/></div><div>RAW<span>MARKET</span></div></div><nav><button className="active">Trade</button><button onClick={()=>setDockTab('balances')}>Portfolio</button><button onClick={()=>setMarketMenu(true)}>Markets</button><button onClick={()=>setMethodology(true)}>Methodology</button></nav><div className="top-actions"><div className={`engine-badge ${engineOnline?'online':'offline'}`}><span/>{engineOnline?'Engine live':'Engine offline'}</div><a className="hedera-badge" href="https://hashscan.io/testnet/contract/0x553678C79D4F38d0C7b1297824048887059AD7BA" target="_blank" rel="noreferrer"><Landmark size={14}/> Hedera testnet <ExternalLink size={11}/></a><button className="wallet-button" onClick={connect} disabled={submitting}><WalletCards size={15}/>{connected?<><span>{wallet.slice(0,7)}…{wallet.slice(-3)}</span><i/></>:'Fund test wallet'}</button></div><button className="mobile-menu" onClick={()=>setMarketMenu(true)}><Menu size={19}/></button></header>

    <section className="market-bar"><button className="market-picker" onClick={()=>setMarketMenu(true)}><CommodityMark symbol={symbol}/><div><strong>{symbol}<small>/ USD</small></strong><span>{market.name}</span></div><ChevronDown size={16}/></button><div className="market-stat primary"><span>MARK</span><strong>{money(book?.last||market.reference)}</strong><small className="positive">Reference aligned</small></div><div className="market-stat"><span>REFERENCE</span><strong>{money(market.reference)}</strong><small>{market.unit}</small></div><div className="market-stat"><span>BEST BID</span><strong className="positive">{book?.bids[0]?money(book.bids[0].price):'—'}</strong><small>{book?.bids[0]?`${book.bids[0].size} available`:'No quote'}</small></div><div className="market-stat"><span>BEST ASK</span><strong className="negative">{book?.asks[0]?money(book.asks[0].price):'—'}</strong><small>{book?.asks[0]?`${book.asks[0].size} available`:'No quote'}</small></div><div className="market-stat"><span>EXPIRY</span><strong>{new Date(market.expiry).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'})}</strong><small>14:00 UTC cutoff</small></div><div className="market-stat"><span>STATUS</span><strong className={market.status.includes('live')?'positive':'warning'}>{market.status}</strong><small>Spot · no leverage</small></div></section>

    <div className="workspace"><div className="workspace-main"><div className="notice-strip"><ShieldCheck size={14}/><span><strong>Source integrity:</strong> charts only display verified benchmark observations. Missing history is never synthesized.</span><button onClick={()=>setMethodology(true)}>View methodology <ArrowUpRight size={12}/></button></div><div className="market-workspace"><CandleChart market={market} data={candles} timeframe={timeframe} onTimeframe={setTimeframe}/><OrderBook book={book} symbol={symbol} onPrice={value=>{setPrice(value.toFixed(2));setOrderType('limit')}}/><section className="trades-card panel"><div className="panel-heading"><div><h2>Recent trades</h2><span>Confirmed engine fills</span></div><Activity size={15}/></div><div className="trade-head"><span>Price</span><span>Size</span><span>Time</span></div><div className="trade-list">{publicTrades.length?publicTrades.map(trade=><div className="trade-row" key={trade.id}><span className={trade.buyer===wallet?'positive':'negative'}>{trade.price.toFixed(2)}</span><span>{trade.qty}</span><span>{stamp(trade.timestamp)}</span></div>):<div className="compact-empty">No fills yet</div>}</div></section></div></div>

      <aside className="order-ticket"><div className="ticket-heading"><div><span>ORDER ENTRY</span><strong>{symbol} spot</strong></div><div className="mode-pill">1×</div></div><div className="side-switch"><button className={side==='buy'?'buy selected':''} onClick={()=>setSide('buy')}><ArrowDownRight size={15}/> Buy</button><button className={side==='sell'?'sell selected':''} onClick={()=>setSide('sell')}><ArrowUpRight size={15}/> Sell</button></div><div className="type-tabs"><button className={orderType==='limit'?'selected':''} onClick={()=>setOrderType('limit')}>Limit</button><button className={orderType==='market'?'selected':''} onClick={()=>setOrderType('market')}>Market</button></div><div className="account-summary"><div><span>Available cash</span><strong>{money(account.available_cash)}</strong></div><div><span>Available {symbol}</span><strong>{position.available-position.reserved}</strong></div></div>{orderType==='limit'?<label className="field"><span>Limit price <small>{market.unit}</small></span><div><input inputMode="decimal" value={price} onChange={event=>setPrice(event.target.value)} placeholder="0.00"/><b>USD</b></div></label>:<div className="market-price"><span>Estimated execution</span><strong>{executablePrice?money(executablePrice):'No liquidity'}</strong><small>Immediate-or-cancel at best displayed price</small></div>}<label className="field"><span>Quantity <small>whole units</small></span><div><input inputMode="numeric" value={qty} onChange={event=>setQty(event.target.value.replace(/\D/g,''))} placeholder="1"/><b>{symbol}</b></div></label><div className="percent-row">{[25,50,75,100].map(value=><button key={value} onClick={()=>setPercent(value)}>{value}%</button>)}</div>{orderType==='limit'&&<label className="check-row"><input type="checkbox" checked={postOnly} onChange={event=>setPostOnly(event.target.checked)}/><span>Post only</span><small>Reject if immediately executable</small></label>}<div className="order-review"><div><span>Estimated value</span><strong>{money(orderValue)}</strong></div><div><span>Fee</span><strong>$0.00 <small>demo</small></strong></div><div><span>Settlement</span><strong>AWS CLOB <Info size={11}/></strong></div></div><button className={`submit-order ${side}`} onClick={submit} disabled={submitting||!engineOnline}>{submitting?<><RefreshCw className="spin" size={16}/> Processing…</>:!connected?'Fund test wallet':`${side==='buy'?'Buy':'Sell'} ${symbol}`}</button><p className="risk-copy">Fully funded spot demo. No leverage, borrowing, liquidation or naked shorting.</p>{toast&&<div className={`toast ${toast.kind}`}>{toast.kind==='success'?<Check size={15}/>:toast.kind==='error'?<CircleAlert size={15}/>:<RefreshCw className="spin" size={15}/>}<span>{toast.message}</span><button onClick={()=>setToast(null)}><X size={13}/></button></div>}</aside>
    </div>

    <section className="portfolio panel"><div className="portfolio-tabs"><button className={dockTab==='orders'?'selected':''} onClick={()=>setDockTab('orders')}>Open orders <b>{openOrders.length}</b></button><button className={dockTab==='fills'?'selected':''} onClick={()=>setDockTab('fills')}>My fills <b>{fills.length}</b></button><button className={dockTab==='balances'?'selected':''} onClick={()=>setDockTab('balances')}>Balances</button><div className="portfolio-value"><span>PORTFOLIO CASH</span><strong>{money(account.cash)}</strong><small>{money(account.reserved_cash)} reserved</small></div></div>{dockTab==='orders'&&<div className="data-table"><div className="table-head"><span>Market</span><span>Side</span><span>Type</span><span>Price</span><span>Filled / Qty</span><span>Status</span><span/></div>{openOrders.length?openOrders.map(order=><div className="table-row" key={order.id}><strong>{order.symbol}/USD</strong><span className={order.side==='buy'?'positive':'negative'}>{order.side.toUpperCase()}</span><span>{order.order_type}</span><span>{money(order.price)}</span><span>{order.filled} / {order.qty}</span><span><i className="open-dot"/> {order.status}</span><button onClick={()=>cancelOrder(order.id)}>Cancel</button></div>):<div className="table-empty"><BookOpen size={19}/><div><strong>No open orders</strong><span>Place a limit away from the spread to see reservation and cancellation.</span></div></div>}</div>}{dockTab==='fills'&&<div className="data-table"><div className="table-head six"><span>Market</span><span>Side</span><span>Price</span><span>Quantity</span><span>Value</span><span>Time</span></div>{fills.length?fills.map(fill=>{const mine=fill.buyer===wallet?'buy':'sell';return <div className="table-row six" key={fill.id}><strong>{fill.symbol}/USD</strong><span className={mine==='buy'?'positive':'negative'}>{mine.toUpperCase()}</span><span>{money(fill.price)}</span><span>{fill.qty}</span><span>{money(fill.price*fill.qty)}</span><span>{stamp(fill.timestamp)}</span></div>}):<div className="table-empty"><Activity size={19}/><div><strong>No fills yet</strong><span>Submit a market order to execute against displayed liquidity.</span></div></div>}</div>}{dockTab==='balances'&&<div className="balance-grid"><div><span>Cash balance</span><strong>{money(account.cash)}</strong><small>Demo USD</small></div><div><span>Available cash</span><strong>{money(account.available_cash)}</strong><small>After reservations</small></div><div><span>Reserved cash</span><strong>{money(account.reserved_cash)}</strong><small>Open buy orders</small></div><div><span>{symbol} inventory</span><strong>{position.available}</strong><small>{position.reserved} reserved</small></div></div>}</section>

    <footer><div><span className={`status-dot ${engineOnline?'':'offline'}`}/><strong>{engineOnline?'AWS engine operational':'Engine unavailable'}</strong><span>· App Runner us-east-1</span></div><div>RawMarket Testnet · Build 0.2.0</div><div><a href="https://github.com/KENILSHAHH/rawmarket" target="_blank" rel="noreferrer">GitHub <ExternalLink size={11}/></a><a href="https://hashscan.io/testnet/contract/0x553678C79D4F38d0C7b1297824048887059AD7BA" target="_blank" rel="noreferrer">HashScan <ExternalLink size={11}/></a></div></footer>

    {marketMenu&&<div className="modal-backdrop" onMouseDown={()=>setMarketMenu(false)}><div className="market-modal" onMouseDown={event=>event.stopPropagation()}><div className="modal-title"><div><span>MARKET DIRECTORY</span><h2>Select a commodity</h2></div><button onClick={()=>setMarketMenu(false)}><X size={18}/></button></div><div className="market-search"><Search size={15}/><input autoFocus value={search} onChange={event=>setSearch(event.target.value)} placeholder="Search markets"/></div><div className="market-list"><div className="market-list-head"><span>Market</span><span>Reference</span><span>Unit</span><span>Status</span></div>{filteredMarkets.map(item=><button key={item.symbol} onClick={()=>chooseMarket(item.symbol)} className={item.symbol===symbol?'selected':''}><div><CommodityMark symbol={item.symbol}/><span><strong>{item.symbol}/USD</strong><small>{item.name}</small></span></div><b>{money(item.reference)}</b><span>{item.unit}</span><em className={item.status.includes('live')?'positive':'warning'}>{item.status}</em></button>)}</div></div></div>}
    {methodology&&<div className="modal-backdrop" onMouseDown={()=>setMethodology(false)}><aside className="methodology-drawer" onMouseDown={event=>event.stopPropagation()}><div className="modal-title"><div><span>CONTRACT SPECIFICATION</span><h2>{market.name}</h2></div><button onClick={()=>setMethodology(false)}><X size={18}/></button></div><div className="methodology-body"><div className="spec-hero"><CommodityMark symbol={symbol}/><div><strong>{symbol}/USD</strong><span>{market.status}</span></div></div><p>{market.description}</p><dl><div><dt>Quotation unit</dt><dd>{market.unit}</dd></div><div><dt>Reference value</dt><dd>{money(market.reference)}</dd></div><div><dt>Contract multiplier</dt><dd>{market.m.toLocaleString()}</dd></div><div><dt>Published cap</dt><dd>{money(market.cap)}</dd></div><div><dt>Trading cutoff</dt><dd>{new Date(market.expiry).toLocaleString()}</dd></div><div><dt>Data quality</dt><dd>{candles.source_status}</dd></div></dl><div className="method-note"><ShieldCheck size={16}/><p>RawMarket keeps the reference index, order-book prices, last fill, portfolio mark and final fixing separate. Market-maker quotes never replace a missing benchmark.</p></div><a href="https://github.com/KENILSHAHH/rawmarket" target="_blank" rel="noreferrer">Read the full methodology <ExternalLink size={13}/></a></div></aside></div>}
  </main>;
}
